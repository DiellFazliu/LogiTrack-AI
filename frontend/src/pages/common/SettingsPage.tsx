...
import { Globe, Mail, Phone, MapPin, Clock, DollarSign, Save } from 'lucide-react';
...
export const SettingsPage: React.FC = () => {
  ...
  setLoading(true);
  try {
    // Check if user is admin AND has an organization
    if (isAdmin && user?.organizationId) {
      ...
      // Non-admin users: use personal profile data
      setSettings({
        name: user?.name || '',
        email: user?.email || '',
        phone: user?.phone || '',
...
const handleSave = async (e: React.FormEvent) => {
  ...
  try {
    if (isAdmin && user?.organizationId) {
      ...
      // Update personal profile (non-admin)
    ...
    await fetchSettings(); // reload fresh data
  } catch (err: any) {
