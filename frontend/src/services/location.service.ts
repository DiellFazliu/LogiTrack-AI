...
export const locationService = {
  ...
  // Merr lokacionin e fundit të ruajtur sipas driver ID
  async getLastSavedLocationByDriverId(driverId: string): Promise<Location | null> {
    try {
      const response = await api.get(`/drivers/${driverId}/location/last`);
      if (response.data && response.data.message) {
        return null;
      }
      return response.data;
